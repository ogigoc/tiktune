# Copied from https://github.com/MiniGlome/Tiktok-uploader/blob/main/Tiktok_uploader.py
...
def uploadVideo(session_id, video, title, tags, schedule_time=0, verbose=True):
    ...
    if not assertSuccess(url, r):
        return False
    r_data = r.json()
    if "message" in r_data and r_data['message'] == 'error':
        if r_data['data']['name'] == 'session_expired':
            raise Exception('Session Expired')
        else:
            raise Exception(r_data['data']['description'])
