...
def main():
    ...
    if videos_per_batch != len(active_accounts):
        print(f"Device videos {videos_per_batch}, active accounts {len(active_accounts)}")
...
def do_campaign(campaign, active_accounts, videos, device):
    print(f"Executing campaign {campaign['name']} with {device['videos_per_campaign'][campaign['name']]} videos")
    ...
    for video_file, account in zip(new_files, active_accounts.itertuples(index=False)):
        ...
        if device['name'] == 'desk':
            ...
            print(f"Trying to upload video on {device['name']}, account: {account.username},{account.name} video: {video_file}")
            try:
